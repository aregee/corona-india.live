<template>
  <div :class="$style.dashboard">
    <vue-grid>
      <vue-breadcrumb
        :items="[{ label: 'Home', href: '/' }, { label: 'World' , href: '/world/stats'}, { label: `${$route.params.country} Statistics`}]"
      />
      <vue-grid-row>
        <vue-grid-item fill>
          <vue-headline level="4">{{$route.params.country}} - Breakdown</vue-headline>
          <br />
          ...
          <home-section>
              ...
              <Query :query="inQuery($route.params)" v-slot="{ data, fetching }">
                <vue-card-body>
                  <vue-donut-chart
                    ...
                    :data="Object.keys(data.country.mostRecent)
                    .map(m => ({key: m, value: data.country.mostRecent[m]}))
                    .reduce((all, item) => {
            ...
            <vue-accordion multiple slot="right">
              ...
              <vue-row-grid>
                <vue-grid-item>
                  <vue-accordion-item title="Refered Links" :initOpen="true" style="max-width:inherit">
...
export default {
  ...
  referedlink(country: "India", state: "${params.country}") {
    link
    source
  }

  helpline(country: "India", state: "${params.country}") {
    number
  }
  labs(country: "India", state: "${params.country}") {
    site
  }
}`;
    },
