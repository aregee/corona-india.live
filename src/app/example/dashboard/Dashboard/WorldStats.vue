<template>
  <div :class="$style.dashboard">
    <vue-grid>
      <vue-breadcrumb :items="[{ label: 'Home', href: '/' }, { label: 'World' , href: '/world/dashboard'}, { label: 'Stats'}]" />
      <vue-grid-row>
        <vue-grid-item fill>
          <vue-headline level="4">Country Wise Data</vue-headline>
          <br/>
          <Query :query="inQuery()" v-slot="{ data, execute }">
            <vue-data-table
              v-if="data"
              :maxRows="10"
              :header="mapHeader(data.countries)"
              :data="mapRows(data.countries)"
              placeholder="Search"
              @click="execute"
            >
            <template v-slot:name="{ cell }">
              <vue-button as="router-link" :target="'/summary/stats/' + cell.value" ghost>{{cell.value}}</vue-button>
            </template>
            </vue-data-table>
          </Query>
          <br />
  ...
  methods: {
    mapHeader(rows: any) {
      return Object.keys({ name: rows[0].name, ...rows[0].mostRecent}).reduce((all:any, item:any) => {
            if(!all[item]) {
              if (['name', 'confirmed', 'growthRate'].includes(item)) {
    ...
    inQuery() {
      return `{countries(names: []){ name, mostRecent { confirmed, deaths, recovered}}}`;
    },
